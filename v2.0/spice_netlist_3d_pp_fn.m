...
if sum(net.param.re_var_max>0)~=0 && ~isempty(strmatch(net.param.write_netlist,'pene'))
    for i=1:length(net.r_var)
        V_node_NaN(net.r_var(i).n1i,net.r_var(i).n1j,net.r_var(i).n1k,:)=NaN;
        V_node_NaN(net.r_var(i).n2i,net.r_var(i).n2j,net.r_var(i).n2k,:)=NaN;
    end
end
...
if net.param.mode~=1
    ...
    V_sns_v_diff=V_sns_v(:,:,:,1)-V_sns_v(:,:,:,dsearchn(net.t_var,1/net.param.r_var_freq(1)/2));
    V_sns_h_diff=V_sns_h(:,:,:,1)-V_sns_h(:,:,:,dsearchn(net.t_var,1/net.param.r_var_freq(1)/2));
end
...
% PTT
% 
% if sum(net.param.re_var_max>0)~=0 && net.param.mode~=1
%     S=length(net.param.re_var_max);
%     for s=1:S
%         r_var_t(s,:,:)=-net.param.re_var_max(s)*sin(2*pi*net.param.r_var_freq(s)*(t_var+net.r_var(s).delayi)-pi/2);
%     end
%     
%     
%     
%     if S==1 && net.param.r_var_dim==3 && net.param.mode~=1
%         for i=1:size(r_var_t,3)
%             [PAT_r_var(i),amp_r_var(i),dc_r_var(i),r_var_fiterr(i)]=find_delay_fn(t_var,squeeze(r_var_t(s,:,i))',net.param.r_var_freq(1),0);
%         end
%         
%         
