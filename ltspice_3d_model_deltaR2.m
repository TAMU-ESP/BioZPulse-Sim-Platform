...
%
%   Simulate arterial pulse amplitude(deltaV_PP) when sensing location is 1.5cm away from the artery
...
for t1=1:length(dia_arr)
    ...
    r_var_n1j=[round((Nj)*(1/2)-r_var_dia/2)];    % R var start location vertical
...
V_s_diff=cat(1,net_t_i_t1_arr.V_s_diff);

figure
p=surf([0:net_t_i_t1_arr.Nj],[0:net_t_i_t1_arr.Ni-1],net_t_i_t1_arr.V_sns_v_diff(:,:,1)*1e3);
view([-26.744347826087 36.5914589488666]);
zlim([-0.05 0.15])
xlabel('X(mm)');ylabel('Y(mm)');zlabel('\DeltaV_{PP}(mV)');
ax = gca; ax.FontSize=13;
set(gcf, 'Position',[403.0000  323.0000  590.0000  381.2000])
