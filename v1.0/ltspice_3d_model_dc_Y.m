...
%
%   Simulate DC Bio-Z(V_DC) for different vertical distance of sensing electrodes 
...
dia_arr=[0];
dia_ar=1;
spacing_x=max(dia_arr)*2+4;
spacing_y=max(dia_arr)+4;
s_num=3;
margin_x=15;
margin_y=6;
...
V_s_arr=cat(1,net_test.V_s);

data=[51.2 33 56];
spacing_data=[1 2 3]*spacing_y-2;
figure
grid on; hold on;
spacing_plot_arr=spacing_arr_in;

for i=1:length(spacing_plot_arr)
    spacing_plot=spacing_plot_arr(i)/res;
    p=dsearchn(net_test.spacing_arr',spacing_plot);
    spacing=net_test.spacing_arr(p);
    V_dc_y=net_test.V_sns_v_space{p}(net.curr.n1i:net.curr.n2i-spacing,net.curr.n1j,1);
    spacing_index=[1:length(V_dc_y)]-1;    
    plot(spacing_index(2:end-1)/1.5,V_dc_y(2:end-1),'linewidth',2)
end
plot(spacing_data/1.5,data/31,'o','linewidth',2)
xlabel({'Voltage Electrodes Location {\itY}_V (cm)'})
ylabel('{\itV}_{DC} (mV)')
legend('Our Model','Measurements','location','north');
ax = gca; ax.FontSize=13;
set(gca, 'FontName', 'Times New Roman');

set(gcf, 'Position',[905.8000  386.6000  487.2000  349.6000])
