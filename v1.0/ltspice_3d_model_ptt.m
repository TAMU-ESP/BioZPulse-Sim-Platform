...
%
%   Simulate PTT and pulse amplitude(deltaV_PP) for different sensing locations relative to the artery
...
i_src_n1j_delta_arr=[0,3,5,7,10,15];
Nj=(max(dia_arr)*dia_ar+2*margin_x)+max(i_src_n1j_delta_arr);
i_src_n1j_arr=round(Nj/2-dia_x/2-max(i_src_n1j_delta_arr)/2)+i_src_n1j_delta_arr;
% Configurations
z_en=0;
nstep=8;
mode=1;  %0=> .TRAN, 1=> .AC
...
for t1=1:length(i_src_n1j_arr)
    clear net_arr2;
    i_src_n1j=i_src_n1j_arr(t1);
    % Sensing    
    i_dia_x=dia_x;
    s_dia_x=dia_x;
    i_dia_y=dia_y;
    s_dia_y=dia_y;
    i_src_n1i=round((Ni-(dia_y+(s_num*2-1+2)*spacing_y))/2);
    ...
    net.param.S=S;
    r_var_delay_arr=10e-3*ones(1,S);
    r_var_dia=2*ones(1,S);
    r_var_n1j=round(i_src_n1j_arr(1)+dia_x/2-r_var_dia/2);    % R var start location vertical
    ...
    if net.param.debug==0
        ...
        if net.param.mode==1 && length(time_arr)>6
            ...
            net_t_i_t1_arr(t1,:)=net_t_r_arr;
        end  
    end
end

for i=3:length(net_t_i_t1_arr)
    net_t_r_arr=net_t_i_t1_arr(i);
    net_t_r=spice_netlist_3d_pp_fn(net_t_r);
    net_t_i_t1_arr(i,:)=net_t_r_arr;
end

V_s_arr=cat(1,net_t_i_t1_arr.V_sns_v_space);
PAT_arr=cat(1,net_t_i_t1_arr.PAT_sns_v_space_n_k1);
V_s_diff_arr=cat(2,net_t_i_t1_arr.V_s_diff);
PAT_V_s_arr=cat(1,net_t_i_t1_arr.PAT_V_s);
curr_arr=cat(1,net_t_i_t1_arr.curr);
param_arr=cat(1,net_t_i_t1_arr.param);

Y_ind=net_t_i_t1_arr(i).curr.n1i+1+net_t_i_t1_arr(i).curr.dia_y:net_t_i_t1_arr(i).curr.n2i-net_t_i_t1_arr(i).spacing_arr-1;
for i=1:length(PAT_arr)
    PAT_i(i,:)=PAT_arr{i}(Y_ind,net_t_i_t1_arr(i).curr.n1j);
    PTT(i)=PAT_i(i,end)-PAT_i(i,1);
    V_sns_v_space_arr=squeeze(net_t_i_t1_arr(i).V_sns_v_space{1}(:,:,1,:));
    V_sns_v_space_diff_arr(i,:)=V_sns_v_space_arr(Y_ind,net_t_i_t1_arr(i).curr.n1j,1)-V_sns_v_space_arr(Y_ind,net_t_i_t1_arr(i).curr.n1j,dsearchn(net_t_i_t1_arr(1).t_var,1/net.param.r_var_freq/2));
end
figure
subplot1=subplot(2,2,1);
hold on;grid on;
plot(Y_ind/10,PAT_i'*1e3,'linewidth',2)
plot(Y_ind/10,mean([net_t_i_t1_arr(i).r_var.delayi(Y_ind+net_t_i_t1_arr(i).spacing_arr/2-1);net_t_i_t1_arr(i).r_var.delayi(Y_ind+net_t_i_t1_arr(i).spacing_arr/2)])*1e3,'--k','linewidth',2)
xlabel({'Vertical Sensing Location Y_V (cm)';'(a)'})
ylabel('Delay of V(t) (ms)')
ax = gca; ax.FontSize=13;
legend(['X_d=0cm';[num2str(([curr_arr(2:end).n1j]-[param_arr(2:end).r_var_n1j]-2)'/10),repmat('cm',length([curr_arr(2:end).n1j]),1)];"Artery"],'Location','NorthWest','fontsize',10);
legend1 = legend(subplot1,'show');
set(legend1,...
    'Position',[0.413783160322955 0.688752596229132 0.106185122324109 0.240726435092354],...
    'FontSize',10);
set(gca, 'FontName', 'Times New Roman');

subplot(2,2,2)
hold on;grid on;
plot(Y_ind/10,V_sns_v_space_diff_arr'*1e3,'linewidth',2)
xlabel({'Vertical Sensing Location Y_V (cm)';'(b)'})
...
subplot(2,2,3)
hold on;grid on;
plot(([curr_arr.n1j]-[param_arr.r_var_n1j]-2)/10,PTT*1e3,'k','linewidth',2)
ylabel('PTT (ms)')
yyaxis right
plot(([curr_arr.n1j]-[param_arr.r_var_n1j]-2)/10,V_sns_v_space_diff_arr(:,1)'*1e3,'linewidth',2)
ylabel(strcat('\DeltaV_{PP} (mV) at Y_V=',num2str(Y_ind(1)/10),'cm'))
xlabel({'Horizontal distance from the artery X_d (cm)';'(c)'})
ax = gca; ax.FontSize=13;
set(gca, 'FontName', 'Times New Roman');

set(gcf, 'Position',[2.6338    0.0866    0.9248    0.5176]*1e3)
file_str=strcat(net_t_i_t1_arr(1).param.path,write_netlist);
