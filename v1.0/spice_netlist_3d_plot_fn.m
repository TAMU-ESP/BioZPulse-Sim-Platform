...
V_node_diff_max=max(V_node_diff_abs(:));
...
if any(fi==net.param.fi_req)
    ...
    subplot(3,2,5)
    heatmap([0:net.Nj],[0:net.Ni],net.V_node(:,:,1,1),'Colormap',parula,'ColorLimits',[-1*V_node_max V_node_max]);
    xlabel('X');ylabel('Y')
    title('Node V (mV)')
    
    k_index=1;
    subplot(3,2,2)
    heatmap([0:net.Nj],[0:net.Ni-1],net.V_sns_v_NaN(:,:,k_index,1),'Colormap',parula,'ColorLimits',[-1*V_sns_max V_sns_max]);
    xlabel('X');ylabel('Y')
