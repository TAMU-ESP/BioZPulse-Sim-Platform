...
%
%   Simulate DC Bio-Z(V_DC) for different electrode spacings 
...
for p=1:length(net_test.spacing_arr)/2
    sns_v_index(p)=length(net_test.spacing_arr)/2-(p)+1;
    spacing_index(p)=net_test.spacing_arr(2*p);
    V_dc_spacing(p)=net_test.V_sns_v_space{2*p}(net.curr.n1i+sns_v_index(p),net.curr.n1j,1);
end

figure
grid on; hold on;
plot([0 spacing_index]/1.5,[0 V_dc_spacing],'linewidth',2)
plot(spacing_data/1.5,data/16,'o','linewidth',2)
xlabel({'Voltage Electrodes Spacing {\itS}_{V} (cm)'})
ylabel('{\itV}_{DC} (mV)')
